...
type CreateItemProps = {
  ...
  onSaveHandler: (item: Omit<Product, 'id' | 'createdAt'>) => void;
};
...
const CreateItemForm = (props: CreateItemProps) => {
  ...
  }, [nameInput, priceInput, categoryInput, quantityInput, descriptionInput]);
  ...
  const quantityInputErrorMessage = quantityInput.hasError ? 'A quantidade é inválida' : 'A quantidade é obrigatória';
  const descriptionInputErrorMessage = descriptionInput.hasError ? 'A descrição é obrigatória' : '';

  const onSaveClick = () => {
    props.onSaveHandler({
      available_quantity: quantityInput.value,
      ...
      <Grid container component={'form'} spacing={3}>
        ...
        <Grid size={{ xs: 12, md: 3, lg: 3 }}>
          <InputField
            ...
            helperText={quantityInputErrorMessage}
